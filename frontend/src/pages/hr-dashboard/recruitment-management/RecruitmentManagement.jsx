...
const RecruitmentManagement = () => {
  ...
  const recruitmentActions = [
    ...
    { title: 'Talent Sourcing Audit', date: 'FY 2024', category: 'Strategic', status: 'Ready', size: 'Verified', path: '/recruitment/sourcing', icon: Target, color: 'text-emerald-600', bg: 'bg-emerald-50' },
  ];
         ...
         <div className="xl:col-span-1 flex flex-col gap-6 overflow-y-auto no-scrollbar pb-10">
            ...
            <div className="card-soft bg-slate-900 p-8 border-none text-white relative overflow-hidden group mt-auto">
               <div className="relative z-10">
                  ...
                  <button className="w-full py-3 bg-white/10 hover:bg-white/20 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all">Review Pipeline</button>
               </div>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar">
               <table className="w-full text-left">
                  <thead className="sticky top-0 z-20 bg-slate-50/80 backdrop-blur-md border-b border-slate-100">
                     <tr>
                        <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Acquisition Workspace</th>
                  ...
                  <tbody className="divide-y divide-slate-50 relative z-10">
                     {recruitmentActions.map((act, idx) => (
                        <tr key={idx} onClick={() => navigate(act.path)} className="group hover:bg-slate-50/50 transition-all cursor-pointer">
                           ...
                           <td className="px-10 py-8 text-right">
                              <div className="flex items-center justify-end gap-3">
                                 <button onClick={(e) => { e.stopPropagation(); showNotification(`Exporting talent pipeline: ${act.title}...`); }} className="p-3 bg-white border border-slate-100 text-slate-400 hover:text-slate-800 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all">
                                    <Download size={20} />
                                 </button>
